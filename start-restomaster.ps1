...
$api = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoExit", "-Command", "cd C:\dev\MILLIONAIRE; pnpm --filter @restomaster/api dev" -PassThru
$web = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoExit", "-Command", "cd C:\dev\MILLIONAIRE; pnpm --filter @restomaster/web dev" -PassThru
